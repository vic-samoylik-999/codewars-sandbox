...
function rot13(str) {
    ...
    for(let i = 0; i < str.length; i++) {
        !str[i].match(re) ? symbols.push(str[i].charCodeAt()) 
            : symbols.push(str[i]);
    }
    const decodedSymbols = symbols.map(item => {
        if(!item.toString().match(re)) {
            return item >= 78 ? String.fromCharCode(item - 13) : String.fromCharCode(item + 13)
        } else {
            return item
        }
    })
    
    return decodedSymbols.join('')
}
